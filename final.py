...
#(["no", "x", "y", "w", "h", "eX", "eY", "eZ", "y_val", "up_val", "fb_vel", "bat", "height", "ft", "roll", "pitch", "yaw", "temp"])
def savetoCSV(no, x, y, w, h, eX, eY, eZ, y_vel, up_vel, fb_vel, batt, height, ft, roll, pitch, yaw, temp):
    with open('datasaved.csv', 'a', newline='') as file:
        writer = csv.writer(file)
        writer.writerow([no, x, y, w, h, eX, eY, eZ, y_vel, up_vel, fb_vel, batt, height, ft, roll, pitch, yaw, temp])
...
def showCam(img, imgsize, tellos, status, data, debug, box, osd, save, video):
    ...
    for (x, y, w, h) in faces:        
        ...
        savetoCSV(counters, x, y, w, h, error[0], error[1], error[2], vel[3], vel[2], vel[1], data[0], data[1], data[2], data[3], data[4], data[5], data[9])
        counters += 1
...
def main():  
    ...
    if args.save is True:
        ...
        videoWriter = cv2.VideoWriter('./video.avi', fourcc, 15, (args.vsize[0],args.vsize[1]))
